...
// Дописать: исправить 8 пунк добавить редактор добавить поиск по критериям цена и количество на складе показать все цена которых ниже
// и колличество которых больше поиск спрашивает цену а потом кол во
#include <iostream>
#include <string>
#include <fstream>
#include <iomanip>
...
public:
	...
	void PrintShellByNameWitnPriceAndCount(double price, int count) {
		...
		Node* p = root;						// вывод
		while (getPrice(p) <= price && getCount(p) >= count) {
			cout << "Article: " << getArticle(p) << " ";
			cout << "Name: " << getName(p) << " ";
			cout << "Count: " << getCount(p) << " ";
			cout << "Prise: " << getPrice(p) << endl;
